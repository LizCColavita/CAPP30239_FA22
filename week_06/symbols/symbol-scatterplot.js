...
d3.csv('penguins.csv').then(data => {
  ...
  svg.append("g")
      .selectAll(".cross")
      .data(data)
      .join("path") //symbols are paths, contrary to circles
      .attr("d", d3.symbol().type(d3.symbolCross).size(12))
      .attr("class", "cross")
      .attr("fill", "orange")
      .attr("opacity", 0.75)
      .attr("transform", d => `translate(${x(d.body_mass_g)},${y(d.flipper_length_mm)})`); //positions the symbol, seen before positioning the axis
